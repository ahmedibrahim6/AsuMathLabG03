...
map<char, CMatrix> mymap;  // a map relates every defined character as matrix
                           // with its data as CMatrix data type
int ErrorFlag = 0;
...
string chartomatrix(
    string matstring);  // takes the very first string coming from file
string concatenate(
    string matstring, int rowcounter,
    int colcounter);  // takes converted string and row and column counter from
                      // the rowcolcounter function
void rowcolcounter(
    string matstring, int& rowcounter,
    int& colcounter);  // takes string after convertion of char to matrix
  ...
  if (argv > 1) {
    ...
    while (getline(infile, s1)) {
      if (s1.find('[') != -1)  // if this line contains matrix declaration
      {
        ...
        string s4, s5;
        bool Semicolon_Detect;  // at the end of line
          ...
          for (int i = 0; i < s1.length(); i++) {
            if (s1[i] == ' ') {
              ...
              if (it != mymap.end())
                ...
                mymap.insert(pair<char, CMatrix>(
                    s1[i], CMatrix(s5)));  // insert new matrix

              if (Semicolon_Detect == false)
                cout << s1[i] << " =" << endl
                     << mymap[s1[i]].getString() << endl
                     << endl;  // print this matrix with its data
              break;
            }
          }

        }
        //  if the declaration in more than one line
        else {
          string s2;
          do {
            getline(infile, s2);
          ...
          for (int i = 0; i < s1.length(); i++) {
            if (s1[i] == ' ')  // we skip spaces and newlines
              ...
              if (it != mymap.end()) {
                ...
                mymap.insert(pair<char, CMatrix>(s1[i], M));
              }  // insert new matrix
              ...
              if (Semicolon_Detect == false) {
                cout << s1[i] << " =" << endl
                     << mymap[s1[i]].getString() << endl
                     << endl;
              }  // print this matrix with its data
          ...
          try {
            ...
            for (int i = 0; i < s2.length(); i++) {
              if (is_char(s2[i])) {
                ...
                if (it != mymap.end() && (it->second).GetnR() == 1 &&
                    (it->second).GetnC() == 1) {
            ...
            for (int i = 0; i < s1.length(); i++) {
              if (s1[i] == ' ')  // we skip spaces and
                ...
                if (it != mymap.end())
                  ...
                  mymap.insert(pair<char, CMatrix>(s1[i],
                                                   CMatrix(s5)));  // insert new
                                                                   // matrix
      ...
      //////////////////////////////////////////// ONE CHAR ON THE LINE OR EMPTY
      /// LINE//////////////////////////////////////////

      else  // if((s1.length()==2))//||(s1.length()==1))   // if one char only
            // in the line without ;
      {
        for (int i = 0; i < s1.length(); i++) {
          map<char, CMatrix>::iterator it;
...
string Get_Result(string s1) {
  int flag = 0,
      FirstOperation =
          0;  // to count the number of the operations between two brackets ()
  char elements[100] =
      {};  // contains the operands of the input string * , - , + , / , ^
  int places[100] = {};     // contains the index of each operand
  string values[100] = {};  // contains all the numbers in the input string in
                            // order in string form
  double Values[100] = {};  // contains all the numbers in the input string in
                            // order in double form
  double
      Result;  // contains the final result of the input string as a double form
  string
      result;  // contains the final result of the input string as a string form
  int j = 0;
  int negelement;
  for (int i = 0; i < s1.length(); i++) {
...
void SinSearch(string& x) {
  ...
  if ((int)z != (-1)) {
    ...
    sub = x.substr(x.find("sin(") + 4,
                   x.find(")", x.find("sin(")) - x.find("sin(") - 4);
    ...
    x.replace(x.find("sin("), x.find(")", x.find("sin(")) - x.find("sin(") + 1,
              '(' + v + /*Mresult*/ ')');
  }
};
...
void CosSearch(string& x) {
  ...
  if ((int)z != (-1)) {
    ...
    sub = x.substr(x.find("cos(") + 4,
                   x.find(")", x.find("cos(")) - x.find("cos(") - 4);
    // CMatrix Mresult=Get_Result(sub);
    // temp=atof(result.c_str());
    // CMatrix c;
    // Mresult.coss();
    ...
    x.replace(x.find("cos("), x.find(")", x.find("cos(")) - x.find("cos(") + 1,
              '(' + v + /*Mresult*/ ')');
  }
};
...
void TanSearch(string& x) {
  ...
  if ((int)z != (-1)) {
    ...
    sub = x.substr(x.find("tan(") + 4,
                   x.find(")", x.find("tan(")) - x.find("tan(") - 4);
    // CMatrix Mresult=Get_Result(sub);
    // temp=atof(result.c_str());
    // CMatrix c;
    // Mresult.tann();
    ...
    x.replace(x.find("tan("), x.find(")", x.find("tan(")) - x.find("tan(") + 1,
              '(' + v + /*Mresult*/ ')');
  }
};
...
void logtrighandle(string& x) {
  ...
  for (int i = 0; i < x.length(); i++) {
    if (x[i] == 's' && x[i + 1] == 'i' && x[i + 2] == 'n') {
      ...
      result = sin(result);
      x.replace(x.find("sin"), temp.length() + 3,
                '(' + to_string(result) + ')');
      ...
      result = atof(temp.c_str());
      result = tan(result);
      x.replace(x.find("tan"), temp.length() + 3,
                '(' + to_string(result) + ')');
      ...
      result = atof(temp.c_str());
      result = cos(result);
      x.replace(x.find("cos"), temp.length() + 3,
                '(' + to_string(result) + ')');
      ...
      if (x[i + 5] == '(') {
        string sub = x.substr(
                   x.find("log10(") + 6,
                   x.find(")", x.find("log10(")) - x.find("log10(") - 6),
               v;
        // CMatrix Mresult=Get_Result(sub);
        // temp=atof(result.c_str());
        // CMatrix c;
        ...
        x.replace(x.find("log10("),
                  x.find(")", x.find("log10(")) - x.find("log10(") + 1,
                  '(' + v + /*Mresult*/ ')');
      } else {
        ...
        result = atof(temp.c_str());
        result = log10(result);
        x.replace(x.find("log10"), temp.length() + 5,
                  '(' + to_string(result) + ')');
      }
    } else if (x[i] == 'l' && x[i + 1] == 'o' && x[i + 2] == 'g' &&
               x[i + 3] != '1') {
      if (x[i + 3] == '(') {
        string sub = x.substr(x.find("log(") + 4,
                              x.find(")", x.find("log(")) - x.find("log(") - 4),
               v;
        // CMatrix Mresult=Get_Result(sub);
        // temp=atof(result.c_str());
        // CMatrix c;
        ...
        v = std::to_string(temp3);
        x.replace(x.find("log("),
                  x.find(")", x.find("log(")) - x.find("log(") + 1,
                  '(' + v + /*Mresult*/ ')');
      } else if (x[i + 3] != '1') {
        while (x[i + 3] >= 48 && x[i + 3] <= 57 || x[i + 3] == '.') {
          temp += x[i + 3];
        ...
        result = atof(temp.c_str());
        result = log(result);
        x.replace(x.find("log"), temp.length() + 3,
                  '(' + to_string(result) + ')');
      }
    }
  }
...
void invsearch(string& x, int y) {
  ...
  for (int i = 0; i < x.length(); i++) {
    if (x[i] == 'a' && x[i + 1] == 's' && x[i + 2] == 'i' && x[i + 3] == 'n') {
      if (x[i + 4] == '(') {
        string sub =
                   x.substr(x.find("asin(") + 5,
                            x.find(")", x.find("asin(")) - x.find("asin(") - 5),
               v;
        // CMatrix Mresult=Get_Result(sub);
        // temp=atof(result.c_str());
        // CMatrix c;
        ...
        x.replace(x.find("asin("),
                  x.find(")", x.find("asin(")) - x.find("asin(") + 1,
                  '(' + v + /*Mresult*/ ')');
      }
        ...
        result = atof(temp.c_str());
        result = asin(result);
        x.replace(x.find("asin"), temp.length() + 4,
                  '(' + to_string(result) + ')');
      }
    }

    else if (x[i] == 'a' && x[i + 1] == 't' && x[i + 2] == 'a' &&
             x[i + 3] == 'n') {
      if (x[i + 4] == '(') {
        string sub =
            x.substr(x.find("atan(") + 5,
                     x.find(")", x.find("atan(")) - x.find("atan(") - 5);
        // CMatrix Mresult=Get_Result(sub);
        //	temp=atof(result.c_str());
        //		CMatrix c;
        //		Mresult.atann();
        ...
        x.replace(x.find("atan("),
                  x.find(")", x.find("atan(")) - x.find("atan(") + 1,
                  '(' + v + /*Mresult*/ ')');
      } else

      {
        ...
        result = atof(temp.c_str());
        result = atan(result);
        x.replace(x.find("atan"), temp.length() + 4,
                  '(' + to_string(result) + ')');
      }
    }

    else if (x[i] == 'a' && x[i + 1] == 'c' && x[i + 2] == 'o' &&
             x[i + 3] == 's') {
      if (x[i + 4] == '(') {
        string sub =
            x.substr(x.find("acos(") + 5,
                     x.find(")", x.find("acos(")) - x.find("acos(") - 5);
        // CMatrix Mresult=Get_Result(sub);
        // temp=atof(result.c_str());
        // CMatrix c;
        // Mresult.acoss();
        ...
        x.replace(x.find("acos("),
                  x.find(")", x.find("acos(")) - x.find("acos(") + 1,
                  '(' + v + /*Mresult*/ ')');
        ...
        result = atof(temp.c_str());
        result = acos(result);
        x.replace(x.find("acos"), temp.length() + 4,
                  '(' + to_string(result) + ')');
      }
    }
  }
...
void hsearch(string& x, int y) {
  if (y == 0) {
    string sub = x.substr(x.find("sinh(") + 5,
                          x.find(")", x.find("sinh(")) - x.find("sinh(") - 5),
           v;
    // CMatrix Mresult=Get_Result(sub);
    // temp=atof(result.c_str());
    // CMatrix c;
    ...
    x.replace(x.find("sinh("),
              x.find(")", x.find("sinh(")) - x.find("sinh(") + 1,
              '(' + v + /*Mresult*/ ')');
  } else if (y == 1) {
    string sub = x.substr(x.find("cosh(") + 5,
                          x.find(")", x.find("cosh(")) - x.find("cosh(") - 5),
           v;
    // CMatrix Mresult=Get_Result(sub);
    // temp=atof(result.c_str());
    // CMatrix c;
    ...
    x.replace(x.find("cosh("),
              x.find(")", x.find("cosh(")) - x.find("cosh(") + 1,
              '(' + v + /*Mresult*/ ')');
  } else if (y == 2) {
    string sub = x.substr(x.find("tanh(") + 5,
                          x.find(")", x.find("tanh(")) - x.find("tanh") - 5),
           v;
    // CMatrix Mresult=Get_Result(sub);
    // temp=atof(result.c_str());
    // CMatrix c;
    ...
    x.replace(x.find("tanh("),
              x.find(")", x.find("tanh(")) - x.find("tanh(") + 1,
              '(' + v + /*Mresult*/ ')');
  }
};
/////////////////////////////////////////////////////exponential & sqrt
...
void expsqrt(string& x) {
  ...
  for (int i = 0; i < x.length(); i++) {
    if (x[i] == 's' && x[i + 1] == 'q' && x[i + 2] == 'r' && x[i + 3] == 't') {
      if (x[i + 4] == '(') {
        string sub =
                   x.substr(x.find("sqrt(") + 5,
                            x.find(")", x.find("sqrt(")) - x.find("sqrt(") - 5),
               v;
        // CMatrix Mresult=Get_Result(sub);
        // temp=atof(result.c_str());
        // CMatrix c;
        ...
        x.replace(x.find("sqrt("),
                  x.find(")", x.find("sqrt(")) - x.find("sqrt(") + 1,
                  '(' + v + /*Mresult*/ ')');
      }
        ...
        result = atof(temp.c_str());
        result = sqrt(result);
        x.replace(x.find("sqrt"), temp.length() + 4,
                  '(' + to_string(result) + ')');
      }
    }

    else if (x[i] == 'e' && x[i + 1] == 'x' &&
             x[i + 2] == 'p')  ////////////////////////warning
    {
      if (x[i + 3] == '(') {
        string sub = x.substr(x.find("exp(") + 4,
                              x.find(")", x.find("exp(")) - x.find("exp(") - 4);
        // CMatrix Mresult=Get_Result(sub);
        //	temp=atof(result.c_str());
        temp1 = atof(sub.c_str());
        //		CMatrix c;
        ...
        x.replace(x.find("exp("),
                  x.find(")", x.find("exp(")) - x.find("exp(") + 1,
                  '(' + v + /*Mresult*/ ')');
      } else

      {
        ...
        result = atof(temp.c_str());
        result = exp(result);
        x.replace(x.find("exp"), temp.length() + 3,
                  '(' + to_string(result) + ')');
      }
    }
  }
...
void trigsearch(string& x) {
  char z = x.find("sin("), z1 = x.find("cos("), z2 = x.find("tan("),
       z3 = x.find("acos"), z4 = x.find("asin"), z5 = x.find("atan");
  string sub = "0", temp;
  ...
  int i = 0;
  while ((x.find("sin", i) != -1 || x.find("cos", i) != -1 ||
          x.find("tan", i) != -1 || x.find("log", i) != -1) &&
         i < x.length()) {
    i++;
    logtrighandle(x);
  }
...
string hesab(string t) {
  ...
  if (ErrorFlag == 1) {
    ...
    string s1 = t.substr(
        t.find('=', 0) + 1,
        t.length() -
            t.find('=', 0));  // gets the string that should be calculated
    string op;
    string R;
    string operand = t.substr(
        0, t.find('=', 0) +
               1);  // the name of the variable that we will store the result in
    for (int i = 0; i < s1.length(); i++) {
      if (s1[i] == ')') {
        int k = i;
        do {
        ...
        op = s1.substr(
            k + 1,
            (i) - (k + 1));  // gets the string that between the brackets ()
        R = Get_Result(op);  // gets the result of the string that between the
                             // brackets and stores it in R
        s1.erase(k, i - k + 1);  // removes the brackets and the string that has
                                 // been calculated
...
void equal_brackets(string s) {
  ...
  for (int i = 0; i < s.length(); i++) {
    if (s[i] == '(') open_bracket_count++;
  }
  for (int i = 0; i < s.length(); i++) {
    if (s[i] == ')') close_bracket_count++;
  }
    ...
    string matstring)  // takes the very first string coming from file
{
       ...
       x1++)  // matstring is the main matrix string
  {
    ...
    if (matstring[x1] == '[') {
      ...
      if (matstring[x1] != '[' && matstring[x1] != ' ' &&
          matstring[x1] != ';' && matstring[x1] != ']' &&
          matstring[x1] != ',') {
        int spacesemibrac = 0;
  ...
  for (int x1 = 0; x1 < matstring.length();
       x1++)  // mat is the main matrix string
  {
    map<char, CMatrix>::iterator it3;
    it3 = mymap.find(matstring[x1]);
    if (it3 != mymap.end()) {
...
string concatenate(string matstring, int rowcounter,
                   int colcounter)  // takes converted string and row and column
                                    // counter from the rowcolcounter function
{
  int OBC1 = 1;
  int CBC1 = 0;
  int r = 0;
